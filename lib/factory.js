...
export default class Factory {
    ...
    addDefault(path) {
        let data = this._resolveFile(path);
        let handler = Handlers.resolveImport(data, this.basedir);
        return this._add(store => handler.then(data => Common.merge(store, data)));
    }

    addOverride(path) {
        let data = this._resolveFile(path);
        let handler = Handlers.resolveImport(data, this.basedir);
        return this._add(store => handler.then(data => Common.merge(data, store)));
    }
