...
const { path: createPath } = require('shortstop-handlers');
...
class Handlers {
            ...
            async.doWhilst(
                function exec(cb) {
                    ...
                    shorty.resolve(data, function (err, result) {
                        if (err) {
                            cb(new Error('Error occured while resolving config with shortstop', {
                                cause: err,
                            }));
                            return;
                        }
                ...
                function complete(err) {
                    if (err) {
                        reject(new Error('Error occured after resolving config is completed', {
                            cause: err,
                        }));
                        return;
                    }
        ...
        return new Promise((resolve, reject) => {
            ...
            shorty.use('import', function (file, cb) {
                try {
                    ...
                    cb(new Error('Error occured during shortstop import in resolveImport', {
                        cause: err
                    }));
                }
            });
            ...
            shorty.resolve(data, function (err, data) {
                if (err) {
                    reject(new Error('Error occured during shortstop resolve in resolveImport', {
                        cause: err,
                    }));
                    return;
                }
    ...
    static resolveCustom(data, protocols) {
        return new Promise(function (resolve, reject) {
            ...
            shorty.resolve(data, function (err, data) {
                if (err) {
                    reject(new Error('Error occured while resolving custom data', {
                        cause: err,
                    }));
                    return;
                }
